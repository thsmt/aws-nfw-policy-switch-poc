...
EventBridge Scheduler を使用すると、Lambda を指定時刻に起動し、ペイロードで `normal` / `patch` の切り替え先を渡せます。

このリポジトリでは、以下の 2 つのスケジュールを作成します。

| スケジュール | ペイロード | 用途 |
| --- | --- | --- |
| `switch-to-patch` | `{"targetPolicy":"patch"}` | メンテナンス前にパッチ運用ポリシーへ切り替え |
| `switch-to-normal` | `{"targetPolicy":"normal"}` | メンテナンス後に通常運用ポリシーへ戻す |
